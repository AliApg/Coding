...
class Bandit:
    ...
    def init_random_samples(self, sample_count):
        means = self.rng.uniform(
            self.mean_range[0], self.mean_range[1], size=(1, self.arms))
        self.optimal_action = np.argmax(means, axis=1)
        self.samples = means + \
            self.rng.normal(0, self.std, size=(sample_count, self.arms))
    ...
    def reset_history(self):
        ...
        self.chose_optimal_history = np.zeros(
            (self.total_plays, self.total_steps), dtype=np.bool8)
    ...
    def select_eps_greedy_action(self, eps, action, step):
        if step == 0:
            action[:] = self.rng.integers(
                0, self.arms, size=(self.total_plays,))
            return
    ...
    def step(self, eps, step, action, ranging):
        ...
        reward = self.samples[self.rng.choice(
            self.arms, size=(self.total_plays,)), action]
        alpha = 1 / (self.arm_pulled[ranging, action] + 1)
        self.ave_reward[ranging, action] += alpha * \
            (reward - self.ave_reward[ranging, action])
...
if __name__ == '__main__':
    ...
    def plot_ave_reward(ave_rewards, epsilons):
        ...
        for i in range(len(ave_rewards)):
            ax.plot(np.mean(ave_rewards[i], axis=0),
                    label=f'eps={epsilons[i]}')
            plt.xlabel('steps')
            plt.ylabel('Reward')
    ...
    def plot_optimal_chosen(chosen_optimal):
        ...
        for i in range(len(chosen_optimal)):
            ax.plot(np.sum(chosen_optimal[i], axis=0) /
                    TOTAL_PLAYS, label=f'eps={epsilons[i]}')
            plt.xlabel('steps')
